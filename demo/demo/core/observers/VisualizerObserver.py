...
class VisualizerObserver(ObserverInterface):
    ...
    def __init__(self, state):
        ...
        self.texts_team_a = [self.ax.text(0, 0, "", 
                                          fontsize=9, 
                                          color="blue", 
                                          weight="bold", 
                                          ha="center") 
        ...
        # Αντίστοιχα για την Team B
        self.texts_team_b = [self.ax.text(0, 0, "", 
                                          fontsize=9, 
                                          color="red", 
                                          weight="bold", 
                                          ha="center") 
    ...
    def update(self, state):
        ...
        for i, robot in enumerate(state["teamA"]):
            ...
            role_name = getattr(robot, "role", f"{robot.robot_id}")
            self.texts_team_a[i].set_text(role_name)
            self.texts_team_a[i].set_position((robot.state[0], robot.state[1] + robot.body_radius + 12))
        ...
        for i, robot in enumerate(state["teamB"]):
            ...
            role_name = getattr(robot, "role", f"{robot.robot_id}")
            self.texts_team_b[i].set_text(role_name)
            self.texts_team_b[i].set_position((robot.state[0], robot.state[1] + robot.body_radius + 12))
