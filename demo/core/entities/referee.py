...
class Referee():
    ...
    def enforce_rules(self,state):
        self.time += 1
        self.check_boundaries_ball(state)
        self.check_boundaries_robots(state)
        self.check_goal(state)
            
        
        
    def check_boundaries_ball(self, state):
        if state["ball"].position[0] >= self.right_field_boundary or state["ball"].position[0] <= self.left_field_boundary or state["ball"].position[1] >= self.upper_field_boundary or state["ball"].position[1] <= self.lower_field_boundary:
    ...
    def check_goal(self,state):
        if state["ball"].position[0] >= state["goalA"].px and state["ball"].position[0] <= state["goalA"].px + state["goalA"].size["length"] and state["ball"].position[1] <= state["goalA"].py + state["goalA"].size["height"]:
            state["score"]["teamB"] += 1
            self.world.reset_positions()
            print("team B scored")
        if state["ball"].position[0] >= state["goalB"].px and state["ball"].position[0] <= state["goalB"].px + state["goalB"].size["length"] and state["ball"].position[1] >= state["goalB"].py:
            state["score"]["teamA"] += 1
            self.world.reset_positions()
            print("team A scored")